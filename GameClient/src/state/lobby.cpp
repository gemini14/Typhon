...
namespace Typhon
{
	...
	enum GUI_ID
	{
		...
		BUTTON_READY, BUTTON_RETURN_MENU,

		STATIC_TEXT_PLAYER, STATIC_TEXT_PLAYER_LIST, STATIC_TEXT_READY
	};
	...
	void Lobby::ChangePlayerReady(const unsigned long playerIP,
			const char ready)
	{
	...
	void Lobby::CheckNewServerCandidate(const LobbyPlayer& newPlayer)
	{
		...
		if (designatedServer.perfScore == 0)
		{
			designatedServer = newPlayer;
				...
				&& GetNetworkIP(designatedServer.sourceAddr)
						< GetNetworkIP(newPlayer.sourceAddr))
		{
			designatedServer = newPlayer;
		}
	}
		...
		BOOST_FOREACH(const LobbyPlayer &p, players)
				{
					...
					if (p.type == HUMAN && p.refreshTime >= PLAYER_TIMEOUT
							&& GetNetworkIP(p.sourceAddr) != network->GetIP())
					{
						playersToRemove.push_back(GetNetworkIP(p.sourceAddr));
						Log(
								ConvertWideToStr(p.name)
										+ " pruned, refresh is "
	...
	Lobby::Lobby(std::shared_ptr<Engine> engine) :
			FSMState(engine), network(
					Typhon::GetNetwork(Typhon::RAW, PORT_NUMBER)), players(
					MAX_PLAYERS, LobbyPlayer()), numBots(MAX_PLAYERS), discoveryMessage(
					ConvertWideToStr(engine->options.name) + " "
									...
									engine->perfScore))
	{
		...
		guiElements.push_back(
				engine->gui->addTabControl(
						core::rect<s32>(
								GUI_ELEMENT_SPACING * 2,
								GUI_ELEMENT_SPACING * 2,
								edgeBorderWidth,
								edgeBorderHeight - BUTTON_HEIGHT
										- GUI_ELEMENT_SPACING), 0, false, true,
						TABCONTROL_PLAYERPANE));

		// Return to main menu button
		guiElements.push_back(
				engine->gui->addButton(
						core::rect<s32>(
								edgeBorderWidth - BUTTON_WIDTH * 2
										- GUI_ELEMENT_SPACING,
								edgeBorderHeight - BUTTON_HEIGHT,
								edgeBorderWidth - BUTTON_WIDTH
										- GUI_ELEMENT_SPACING,
								edgeBorderHeight), 0, BUTTON_RETURN_MENU));
		engine->lang->AddElementWithText(guiElements.back(), "BackToMainMenu");

		// Ready button
		guiElements.push_back(
				engine->gui->addButton(
						core::rect<s32>(edgeBorderWidth - BUTTON_WIDTH,
								edgeBorderHeight - BUTTON_HEIGHT,
								edgeBorderWidth, edgeBorderHeight), 0,
						BUTTON_READY));
		engine->lang->AddElementWithText(guiElements.back(), "Ready");

		// "Player" column label
		guiElements.push_back(
				engine->gui->addStaticText(
						L"",
						core::rect<s32>(GUI_ELEMENT_SPACING * 3,
								GUI_ELEMENT_SPACING * 2 + 7,
								GUI_ELEMENT_SPACING * 3 + 50,
								GUI_ELEMENT_SPACING * 2 + 27), false, false, 0,
						STATIC_TEXT_PLAYER));
		...
		playersGUI = engine->gui->addStaticText(
				L"",
				core::rect<s32>(GUI_ELEMENT_SPACING * 3,
						GUI_ELEMENT_SPACING * 4, GUI_ELEMENT_SPACING * 3 + 200,
						edgeBorderHeight - BUTTON_HEIGHT - GUI_ELEMENT_SPACING),
				false, true, 0, STATIC_TEXT_PLAYER_LIST);
		guiElements.push_back(playersGUI);

		for (float i = 2.5f; i < 2.5f + MAX_PLAYERS; ++i)
		{
			auto readyImg = engine->gui->addStaticText(
					L"",
					core::rect<s32>(
							static_cast<int>(edgeBorderWidth
									- GUI_ELEMENT_SPACING * 4.5), static_cast<int>(CHECKBOX_SPACING * i),
			...
			const unsigned long location)
	{
		...
		sort(players.begin(), players.end(),
				[](const LobbyPlayer& lhs, const LobbyPlayer& rhs)
				{
		...
		if (event.EventType == EET_GUI_EVENT)
		{
			...
			switch (event.GUIEvent.EventType)
			{
				...
				case gui::EGET_COMBO_BOX_CHANGED:
					...
					engine->lang->ChangeLanguage(
							static_cast<LANG>(engine->lang->langSelector->getSelected()));return true;
	...
	{
		auto iter = find_if(players.begin(), players.end(), [=](LobbyPlayer p)
		{	return GetNetworkIP(p.sourceAddr) == addr;});
		if (iter != players.end())
		{
	...
	void Lobby::Run()
	{
		...
		if (recvMessage.prefix == 'D')
		{
			...
			if (iter == players.end())
			{
				...
				AddPlayer(newPlayerName, perf,
						GetNetworkIP(recvMessage.address));
			}
			else
			{
				iter->refreshTime = 0;
		...
		else if (recvMessage.prefix == 'R')
		{
			...
			if (iter != players.end())
			{
				ChangePlayerReady(GetNetworkIP(iter->sourceAddr),
						recvMessage.msg[0]);
			}
		}
		...
		if (ReadyToPlay())
		{
			...
			// if this client is the server, make a note of it
			if (network->GetIP() == GetNetworkIP(designatedServer.sourceAddr))
			{
				engine->clientIsServer = true;
				auto generateLobbyList =
						[=]() -> string
						{
							string list;
							BOOST_FOREACH(const LobbyPlayer p, players)
				...
				engine->lobbyList = std::shared_ptr<string>(new string(generateLobbyList()));
			}
